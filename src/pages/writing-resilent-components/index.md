...
С одной стороны - это хорошая идея иметь последовательные правила по всему проекту, но с другой стороны, многие из этих правил произвольны и поэтому в React\`e нет строгих правил на это счёт.

Вы можете использовать разные системы типизации, предпочитать объявление функций (function declarations) или стрелочные функции, распологать ваши пропсы в алфавитном порядке или в любом другом, который вам нравится. 
...
Прежде чем мы поговорим о принципах разработки компонентов, я хочу сказать пару слов о стайлгайдах. Пусть это не самое популярное мнение, но кто-то должен его озвучить!

В JavaScript сообществе, есть несколько строгих и упрямых стайлгайдов навязываемых линтером. Моё личное наблюдение: линтеры склонны создавать больше сложностей, чем они решают. Я не смогу вспомнить как много раз кто-то показывал мне абсолютно валидный код и говорил при этом: "React жалуется на это", но на самом деле причина была в конфиге их линтера! Это приводит к трём проблемам:

* Люди привыкли видеть в линтерах скорее **надзирателя**, а не полезный инструмент. Полезные замечания тонут среди советов о стиле кода. Как результат, люди не просматривают сообщения от линтера во время дебага и пропускают действительно полезные заметки. Дополнительно, люди которые меньше привыкли писать на JavaScript (например, дизайнеры) будут испытывать ещё больше трудностей при работы с кодом. 
...
* В итоге, люди принимают эту насаждённую линтерами установку и используют навязанное им мнение о вещах которые хоть и легко сканируемы в коде, но **не несут существенной разницы**. "Вы использовали декларирование функций, но в **нашем** проекте мы используем стрелочные функции". Всякий раз, когда я должен соблюдать подобное правило, я испытываю сильные чувства, глубокий взгляд показывает, что я вложил эмоциональные усилия в это правило и изо всех сил стараюсь его отпустить. Это заманивает меня в ловушку ложного чувства выполненного долга без действительного улучшения моего кода.
...
Конечно есть аспекты в код-стайле, которые напрямую не относятся к форматированию, но всё равно могут раздражать, если непоследовательно встречаются по всему проекту.
...
Не стоит всё автоматизировать! Знания полученные из **фактического чтения** в таком гайде могут быть более ценными чем следование "правилам".
...
1. **[Не останавливайте поток данных](#принцип-1-не-останавливайте-поток-данных)**
2. **[Всегда будьте готовы к ререндеру](#принцип-2-всегда-будьте-готовы-к-рендеру)**
3. **[Компонент не должен быть синглтоном](#принцип-3-компонент-не-должен-быть-синглтоном)**
4. **[Держите локальный стейт изолированным](#принцип-4-держите-локальный-стейт-изолированным)**

Даже если если вы не используете React, вы скорее всего откроете для себя те же самые принципы путём проб и ошибок, для любой UI компонентной модели с однонаправленным потоком данных.
...
Но обычно вы хотите только **прочитать свойства напрямую в вашем компоненте** и избегаете копирование свойств (или чего-то другого вычесленного из пропсов) в стейт.
...
* Убедимся, что когда эти свойства изменяются, мы перезапускаем сайд-эффект.
	- Мы можем сделать это изменив метод `componentDidUpdate`.
...
К сожалению, такая автоматическая проверка компонента написанная через классы слишком сложна. Любой метод может быть вызван любым другим. Cтатический анализ вызванный из `componentDidMount` и `componentDidUpdate` чреват ложно положительными результатами.

Тем не менее, *можно* разработать такое API которое *сможет* статически анализировать целостность. Пример такого API - [React useEffect Hook](https://overreacted.io/a-complete-guide-to-useeffect/):
...
![Дэмо работы линтера](./useeffect.gif)
...
**Заметьте, важно внимательно относиться ко всем изменениям пропсов и стейта для сайд-эффектов, вне зависимости пишите вы комонет как класс или как функцию.**

С API классов вы должны думать о целостности сами и убедиться в том, что определённое изменения по отоношению к каждому соответствующему свойству или стейту обрабатываются в `componentDidUpdate`. Иначе, ваш компонент не будет гибким к изменениям пропсов и стейта. Это проблема не является специфичной только для React`а, она касается любой UI библиотеки, которая позволяет вам обрабатывать “создание” и “обновление” отдельно.

**`useEffect` API переворачивает наше обычное представление об обновлении компонента, поощряя последовательность.** [По началу это может казаться вам непривычным](https://overreacted.io/a-complete-guide-to-useeffect/), но как результат ваши компоненты будут более гибкими к изменениям в логике. По причине того, что “зависимости” теперь стали более явными, мы можем *верифицировать* консистентность эффекта используя правило линтера. Мы используем правила линтера для отловли багов!
...
**Но если вы попытаетесь “оптимизировать” компонент написанием  своего сравнения, вы можете забыть сравнить функции, передаваемые через пропсы:**
...
Это может ещё больше запутать если сущность функции сама по себе зависит от чего-либо, что со временем может измениться, как `draft.content` в примере:
...
**Как нам избежать этой пролемы?**

Я рекомендую избегать ручной имплементации `shouldComponentUpdate` и кастомного сравнения в `React.memo()`.
...
Если вам действительна нужно кастомное сравнение, **убедитесь, что в нём вы не пропустите функции:**
...
Как я упоминал ранее, легко упустить это проблему в классах, потому что методы часто стабильны (но не всегда - и вот здесь баги становится тяжело отлаживать). С хуками ситуация иная:
...
Всякий раз когда вы используете пропсы и стейт, подумайте о том, что должно произойти, если они изменятся. В большинстве случаев, компонент не должен относиться к первому и последующим рендерам по-разному. Это делает его гибиким к изменениям в логике.
...
Не пытайтесь вводить ненужные предположения о времени в поведение вашего компонента. **Ваш компонент должен быть готов к ре-рендеру в любой момент.**
...
Или  вы можете использовать неконтролируемый компонент с key что бы сбросить его состояние:
...
**Не оставляйте этот код** - это лишь быстрый способ проверить, что будет происходить, когда родитель будет рендериться чаще чем вы ожидаете. Это недолжно сломать потомка!
...
Но это дает нам ложное чувство стабильности. **Этот компонент по прежнему не гибок к _действительным_ изменениям пропсов.** Например, если мы добавим *ещё одно* часто меняющееся свойство, такое как анимированный `стиль`, мы по-прежнему будем “терять” весь стейт:
...
Неважно, планируете ли вы рендерить этот компонент дважды или нет, решение этих проблем окупится в долгосрочной перспективе. Это приведёт вас к более гибкому варианту проектирования.
...
Например, представьте мы дважды отрендерели `Post`. Давайте взглянем на разные вещи внутри него которые могли измениться.

* *Контент Post`а.* Мы хотим редактировать пост в одном дереве, что бы обновить его в другом. Вероятно, **не должно** быть локального стейта у компонента `Post`. (Вместо этого, контент поста лучше разместить в каком-нибудь кэше вроде Apollo, Relay или Redux.)


* *Список комментариев.* Здесь схожая ситуация с предыдущей. Мы хотим добавить новый коммент в одно дерево комментариев, так что бы он появился в другом дереве. Идеально, если мы будем использовать какой-то кэш для этого, и он **не должен** быть локальным стейтом нашего Post`а.


* *Какие комментарии развёрнуты.* Было бы странным, если бы развернув комментарий в одном дереве, мы развернули бы его и в другом. В этом случае мы скорее взаимодействуем с конкретным *UI представлением* компонента `Comment`, чем с абстрактной “сущностью комментария”. Следовательно, флаг “расширения” *должен* быть локальным стейтом компонента `Comment`.

* *Значение нового комментария из инпута.* Будет непонятно, если написание коммента в одном инпуте так же обновило бы инпут в другом дереве. Если только инпуты чётко не сгруппированы вместе, обычно люди ожидают, что они независимы. Значение из инпутов **должно** быть локальным стейтом компонента `NewComment`.

Я не предлагаю догматической интерпретации этих правил. Конечно, в маленьких приложениях вы можете захотеть использовать локальный стейт для всего, включая эти “кэши”. Я только говорю об идеальном пользовательском опыте [из первых принципов](/post/the-elements-of-ui-engineering/).

**Избегайте создания из локального стейта глобального.** Это подходит к нашей теме “гибкости”: между компонентами меньше неожиданных синхронизаций. Как бонус, это *также* фиксит большой класс проблем связанных с производительностью. Частый ре-рендер намного меньшая проблема, когда ваш стейт находится в правильном месте.
...
1. **[Не останавливайте поток данных](#принцип-1-не-останавливайте-поток-данных)** Пропсы и стейт могут меняться, но компонент должен обработать эти изменения, когда бы они не произошли.
2. **[Всегда будьте готовы к ререндеру](#принцип-2-всегда-будьте-готовы-к-рендеру)** Компонент не должен сломаться из-за того что он рендерится чаше или реже.
3. **[Компонент не должен быть синглтоном](#принцип-3-компонент-не-должен-быть-синглтоном)** Даже если компонент отрендерился только один раз, ваша разработка улучшится, если двойной рендер не сломает ваш компонент. 
4. **[Держите локальный стейт изолированным](#принцип-4-держите-локальный-стейт-изолированным)** Думайте о том какой стейт будет локальным для конкретного UI представления - и не поднимайте стейт выше чем необходимо.
